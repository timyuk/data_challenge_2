...
sprintf("Fixed individual/borough effect: %s", 
        pFtest(pooled_borough, pooled_baseline)$p.value <= 0.05)
...
## Test presence of random individual (borough) and time effects (Breush-Pagan)
sprintf("Random individual effect: %s", plmtest(cpooled_baseline, type="bp", 
                                                effect="individual")$p.value <= 0.05)
...
# Compare baseline models to those that take (some) effects into account
sprintf("Fixed individual/borough effect: %s", 
        pFtest(upooled_borough, upooled_baseline)$p.value <= 0.05)
...
print(paste("R-squared (individual effects):", r2_ufixed_ind))


### CONCLUSION ON FINAL MODEL FOR UNDERREPORTING
summary(ufixed_ind_model)
# R2 very low (0.15 and 0.09). Small proportion of variance of underreporting 
